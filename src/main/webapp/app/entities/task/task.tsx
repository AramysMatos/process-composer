  ...
  return (
    <div>
      <h2 id="task-heading" data-cy="TaskHeading">
        <Translate contentKey="processComposerApp.task.home.title">Tasks</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="processComposerApp.task.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/task/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="processComposerApp.task.home.createLabel">Create new Task</Translate>
          </Link>
        </div>
      </h2>
        ...
        {taskList && taskList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="processComposerApp.task.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {taskList.map((task, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{task.gitHubUrl}</td>
                  <td>{task.gitHubNodeId}</td>
                  <td>
                    {task.activities
                      ? task.activities.map((val, j) => (
                          <span key={j}>
                            <Link to={`/activity/${val.id}`}>{val.name}</Link>
                            {j === task.activities.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="processComposerApp.task.home.notFound">No Tasks found</Translate>
            </div>
          )
        )}
      </div>
